...
tui_traffic_menu() {
    while true; do
        ...
        echo -e "  ${BOLD}Текущая сессия${NC} ${DIM}(через метрики telemt)${NC}:"
        ...
        if [ "$_reanimator" = "true" ]; then
            ...
            if _json=$(_get_telemt_users_json 2>/dev/null); then
                _labels=$(grep -oE '"username"[[:space:]]*:[[:space:]]*"[^"]*"' <<< "$_json" | sed -E 's/.*"([^"]*)"$/\1/')
            fi
            if [ -z "$_labels" ]; then
                echo -e "  ${DIM}API цели недоступно/выключено — детализация по пользователям недоступна${NC}"
            else
                local label
