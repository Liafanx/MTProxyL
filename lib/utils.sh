...
# Тот же экранированный результат, но кладёт его в $_JSON_ESCAPE_OUT вместо
# stdout — для циклов на N секретов/строк, где "$(json_escape ...)" означает
# N форков подпроцесса на пустом месте (сам json_escape ничего не запускает,
# но $(...) всё равно форкает подшелл, чтобы поймать вывод). local -n не
# используем: минимум для MTProxyL — bash 4.2, неймреф появился в 4.3.
_JSON_ESCAPE_OUT=""
json_escape_fast() {
    local _s="$1"
    _s="${_s//\\/\\\\}"
