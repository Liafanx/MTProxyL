...
#
# В режиме менеджера config.toml примонтирован в контейнер движка только для
# чтения: telemt не может записать его сам и отвечает «Device or resource
# busy». Настройки прокси живут в settings.conf и принадлежат MTProxyL,
# поэтому панель меняет их здесь, а не через API движка.
#
# Формат каталога: "КЛЮЧ|валидатор|описание". Валидаторы те же, что у
# экспертного каталога (_expert_validate), чтобы правила проверки были одни
# на весь проект.
...
#
# У части ключей смена тянет за собой побочные действия — правила гео-блокировки
# прибиты к порту, длина поддельного сертификата зависит от хоста маскировки, —
# и они уже реализованы в готовых командах. Такие ключи отдаём им, а не
# присваиваем значение напрямую: иначе панель молча оставила бы систему в
...
settings_set_param() {
    ...
    # Раздельными объявлениями намеренно: в одном `local a=... b="${a...}"`
    # bash сперва объявляет оба имени (обнуляя их), и b получает пустое
    # значение. Пустой валидатор _expert_validate пропускает всё подряд —
    # проверка значений отключилась бы целиком и молча.
