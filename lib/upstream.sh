...
# Область применения (scopes) из мануала движка: список тегов через запятую.
# Если у запроса есть scope — берутся только апстримы, у которых этот тег есть
# в scopes. Если scope у запроса нет — берутся ТОЛЬКО апстримы с пустым scopes.
# Отсюда правило: хотя бы один включённый маршрут обязан оставаться без scopes,
# иначе обычному трафику не достанется ни одного.
