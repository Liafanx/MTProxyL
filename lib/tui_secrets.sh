...
tui_secrets_menu() {
    while true; do
        ...
        case "$choice" in
            1)
                echo -en "  ${BOLD}Метка:${NC} "; local l; read -er l
                [ -n "$l" ] && { secret_add "$l" || true; }; press_any_key ;;
            2)
                echo -en "  ${BOLD}Метка или #:${NC} "; local l; read -er l
            ...
            5)
                ...
                if [[ "$l" =~ ^[0-9]+$ ]] && [ "$l" -ge 1 ] && [ "$l" -le "${#SECRETS_LABELS[@]}" ]; then
                    l="${SECRETS_LABELS[$((l - 1))]}"; fi
                if [ -n "$l" ]; then
                    echo -en "  ${BOLD}Макс. соединений (0=∞):${NC} "; local mc; read -er mc
                    echo -en "  ${BOLD}Макс. IP (0=∞):${NC} "; local mi; read -er mi
                    echo -en "  ${BOLD}Квота (напр. 5G, 0=∞):${NC} "; local dq; read -er dq
                    echo -en "  ${BOLD}Срок (YYYY-MM-DD, 0=нет):${NC} "; local ex; read -er ex
                    secret_set_limits "$l" "${mc:-0}" "${mi:-0}" "${dq:-0}" "${ex:-0}" || true
                fi; press_any_key ;;
            6)
                echo -en "  ${BOLD}Источник:${NC} "; local s; read -er s
                if [[ "$s" =~ ^[0-9]+$ ]] && [ "$s" -ge 1 ] && [ "$s" -le "${#SECRETS_LABELS[@]}" ]; then
                    s="${SECRETS_LABELS[$((s - 1))]}"; fi
                echo -en "  ${BOLD}Новая метка:${NC} "; local n; read -er n
                [ -n "$s" ] && [ -n "$n" ] && { secret_clone "$s" "$n" || true; }; press_any_key ;;
            7)
                echo -en "  ${BOLD}Старая:${NC} "; local o; read -er o
                if [[ "$o" =~ ^[0-9]+$ ]] && [ "$o" -ge 1 ] && [ "$o" -le "${#SECRETS_LABELS[@]}" ]; then
                    o="${SECRETS_LABELS[$((o - 1))]}"; fi
                echo -en "  ${BOLD}Новая:${NC} "; local n; read -er n
            ...
            10)
                ...
                if [ -n "$l" ]; then
                    ...
                    if [ $idx -ge 0 ]; then
                        ...
                        echo -en "  ${BOLD}Новый ключ (32 hex):${NC} "; local nk; read -er nk
            ...
            12)
                echo -en "  ${BOLD}Файл для импорта:${NC} "; local f; read -er f
                if [ -f "$f" ]; then
