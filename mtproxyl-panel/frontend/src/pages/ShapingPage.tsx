...
export function ShapingPage() {
  ...
  const personal = form.mode === 'manual' ? form.manual_ip_mbps : total / divisor;
  const validation = useMemo(() => {
    ...
    if (form.manual_ip_mbps < 0.1 || form.manual_ip_mbps > form.manual_total_mbps) return 'Лимит IP: от 0,1 Мбит/с до общего потолка';
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {form.mode === 'manual' ? <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-1"><Label htmlFor="shaping-ip">На один IPv4, Мбит/с</Label><Input id="shaping-ip" type="number" min="0.1" step="0.1" value={form.manual_ip_mbps} onChange={setNumber('manual_ip_mbps')} /></div>
          ...
          <p className="text-sm text-text-secondary">Предварительно: общий потолок {total.toLocaleString('ru-RU')} Мбит/с; на один IP {personal.toLocaleString('ru-RU', { maximumFractionDigits: 2 })} Мбит/с{form.mode === 'dynamic' ? ` при делителе ${divisor}` : ''}.</p>
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-1"><Label htmlFor="shaping-profiles">Профили без лимита на IP</Label><Input id="shaping-profiles" placeholder="admin, trusted" value={profiles} onChange={(e) => setProfiles(e.target.value)} /><p className="text-xs text-text-secondary">Через запятую. IP профиля остаются внутри общего потолка; если один IP используют разные профили, действует лимит.</p></div>
