...
// ExpertParam is one entry of MTProxyL's telemt parameter catalog. The catalog
// is the authority on validation; the panel renders a field from Validator.
...
var (
	...
	// Values cover integers, booleans, enum words, domains, URLs, addresses and
	// comma-separated lists. Brackets and quotes stay out deliberately: list
	// params are entered plain and get TOML array syntax on apply, and a quote
	// would let a string param produce key = "va"lue".
...
// SetExpertParam stores an override for one engine parameter. The write is
// deferred: callers save a batch, then call ApplyExpert once.
...
// SuperExpertStatus describes the hand-authored engine config mode. While it is
// active MTProxyL stops generating the config, so the panel disables what would
// be overwritten.
...
// SuperExpertRead returns the hand-authored config verbatim: stripping ANSI
// would corrupt a config that legitimately contains such sequences.
func (c *Client) SuperExpertRead(ctx context.Context) (string, error) {
	return c.run(ctx, "superexpert", "show")
}

// SuperExpertWrite replaces the hand-authored config. Content goes over stdin:
// multi-line TOML with quotes is awkward and risky as an argument.
...
// PQCheck probes a domain for post-quantum key exchange support. iOS clients
// can hang on «Connecting…» against a FakeTLS domain without it.
