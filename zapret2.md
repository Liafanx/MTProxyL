...
Если они прежние — ключи задаёт файл, который читается после нашего. Порядок
такой: сначала `/etc/sysctl.d/*.conf` по возрастанию имён, в самом конце —
`/etc/sysctl.conf`. То есть последний выигрывает всегда, и переименованием
файла в `sysctl.d` его не обойти. Найдите источник и уберите оттуда эти строки:
...
### Не подключается один клиент — обход на его роутере

Если сервер в порядке, а не подключается кто-то один, посмотрите на его сторону:
на домашнем роутере (Keenetic, OpenWrt, Padavan) часто стоит свой обход на том
же `nfqws`. Он собирает ClientHello из сегментов, чтобы прочитать SNI, а мы
намеренно шлём его рваным и с битой контрольной суммой — собрать такой поток
нельзя, и роутер ломает соединение уже сам. Лечится **на роутере**, любым из двух:

- параметр запуска `nfqws` **`--reasm-disable=tls_client_hello`** — сборка
  ClientHello выключается;
- домен маскировки прокси (тот же, что в SNI FakeTLS) — в **исключения**
  `nfqws`, тогда до него роутер не вмешивается.

Второй вариант точечнее: обход на роутере продолжает работать для остального
трафика. Быстрая проверка догадки — временно выключить обход на роутере целиком.
