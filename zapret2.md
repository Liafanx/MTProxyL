...
в SYN и уходят через `fwmark` без вмешательства: описанная ниже схема им ломает
соединение. Остальные идут дальше по сценарию.
...
влезает, и ядро Linux отправляет примерно половину окна — в примере ниже 700 байт.
