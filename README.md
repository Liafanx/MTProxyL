...
При смене `PROXY_PORT` правила zapret2, SYN-лимитера и гео-блокировки
переезжают на новый порт сами.

### Что нужно

- **Свой домен.** В раскладке `shared` он обязан отличаться от домена маскировки:
...
| Carrier | Когда брать |
| --- | --- |
| `https-lanes` | По умолчанию. Отдельные потоки не блокируют друг друга, нужен HTTP/2 на фронте |
...
принципе**, и на практике это заметно режет ему скорость.

SYN-лимитер, наоборот, работает с WEB отлично: `https-lanes` мультиплексирует
потоки поверх одного-двух TCP-соединений, и в ставку лимитера они не упираются.

Поэтому при включённом zapret2 и раскладке `shared` MTProxyL предупреждает и
предлагает два выхода: перейти на раскладку `split`, где фильтр идёт по порту
прокси и WEB не задевается, либо заменить zapret2 SYN-лимитером.
...
обычными ссылками, кнопкой и текстом.
