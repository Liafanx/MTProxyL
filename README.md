...
| Лимиты CPU и памяти | `--cpus`, `--memory` | средствами systemd |
...
| `--cpus` | число, до числа ядер машины | без ограничений | Лимит CPU для контейнера. Больше, чем ядер, docker не примет — установка откажется заранее. С `--engine binary` не принимается: у бинарника лимитов контейнера нет. |
| `--memory` | `512m`, `1g` | без ограничений | Лимит памяти для контейнера. С `--engine binary` не принимается. |
