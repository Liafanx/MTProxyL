...
- **Сайт-заглушка из одного каталога.** Движок собирает его снимок при старте
  процесса — файлы, добавленные позже, он отдаёт как 404, пока не перезапустить.
  ...
  по другому — нет». Во встроенных шаблонах MTProxyL стили и скрипты выносятся
  в отдельные файлы: строгая Content Security Policy WEB запрещает inline-код.
...
| Carrier | Когда брать |
| --- | --- |
| `websocket-lanes` | **По умолчанию.** Отдельный сокет на каждый поток. Единственный, кто нормально работает вместе с zapret2 |
