...
Проще так: сначала B, и если разведка не находит живых эндпоинтов — значит wg
режут, берите A.

### Middle proxy с WARP не работает

Ключи ME-рукопожатия обе стороны выводят из адреса **и порта**, с которых прокси
пришёл к Telegram. Выход WARP — общий CGNAT Cloudflare, он меняет и адрес, и
порт: стороны считают разные ключи, ни один писатель не встаёт, покрытие
дата-центров падает до нуля. Подстановка выходного адреса в
`middle_proxy_nat_ip` не спасает — порт всё равно чужой.
