...
- [Что умеет](#что-умеет)
- [Основные CLI команды](#основные-cli-команды)
  - [Прокси](#прокси)
  - [Секреты (пользователи)](#секреты-пользователи)
  - [Настройки](#настройки)
  - [Движок Telemt](#движок-telemt)
  - [Режим эксперта](#режим-эксперта)
  - [NFT SYN Limiter](#nft-syn-limiter)
  - [Безопасность](#безопасность)
  - [Мониторинг](#мониторинг)
  - [Бэкапы и обновления](#бэкапы-и-обновления)
  - [Система](#система)
- [NFT SYN Limiter — подробнее](#nft-syn-limiter--подробнее)
- [iOS фиксы](#ios-фиксы)
- [Режим эксперта — подробнее](#режим-эксперта--подробнее)
- [Модульная архитектура](#модульная-архитектура)
- [Требования](#требования)
- [Отличия от MTProxyMax](#отличия-от-mtproxymax)
- [Удаление](#удаление)
...
> **Важно:** для стабильной работы прокси на текущий момент рекомендуется включить NFT SYN limiter — без него подключение клиентов нестабильно в ~90% случаев. Лимитер включается в меню `[7] NFT лимитер и iOS фиксы` или через CLI: `mtproxyl nft apply`.
...
- Детальные метрики движка (соединения, upstream, ME health, users, security)
...
mtproxyl sni-policy mask               # Установить SNI-политику (mask/drop)
...
```bash
mtproxyl nft apply            # Применить NFT правила
mtproxyl nft remove           # Удалить NFT правила
mtproxyl nft preset hard      # Пресет: жёсткий (1/s burst 1)
mtproxyl nft preset medium    # Пресет: средний (1/s burst 3)
mtproxyl nft preset soft      # Пресет: мягкий (2/s burst 5)
mtproxyl nft service          # Установить systemd-службу (автозапуск)
mtproxyl nft drop             # Счётчик дропов (live)
...
Основной метод стабилизации подключений — **ограничение входящих SYN-пакетов отдельно для каждого IP клиента** через nftables:
...
| **Средний** | 1/second | 3 | Если жёсткий слишком строг |
| **Мягкий** | 2/second | 5 | Для серверов с большим числом клиентов |

### Systemd-служба

Правила автоматически применяются при загрузке, если установлена служба:

```bash
mtproxyl nft service    # Установить и включить
```
...
Если клиенты за CGNAT (общий IP) или VPN — жёсткий режим может давать ложные срабатывания. В таких случаях используйте средний или мягкий пресет.
...
Ускоряет обнаружение мёртвых сокетов через `sysctl`. Подходит когда iOS-клиенты после фона/сна не могут нормально переподключиться.
...
│   ├── utils.sh                 # Утилиты, валидация, автообновление
...
│   ├── tui_nft.sh               # Подменю: NFT limiter + iOS
...
| Модульная архитектура | ✅ | ❌ (1 файл 8000+ строк) |
| Управление версиями Telemt | ✅ (list / update / rollback) | ❌ |
| Режим эксперта с каталогом | ✅ (все параметры telemt с валидацией) | ❌ |
| NFT SYN limiter | ✅ | ❌ |
| iOS фиксы (v1 + v2) | ✅ | ❌ |
| Автовыбор свободного metrics port | ✅ | ❌ |
