...
TLS движок не терминирует. Публичный порт 443 забирает nginx, разбирает SNI и
разводит трафик:
...
клиент :443
  └─ nginx, ssl_preread
       ...
       └─ всё остальное   → FakeTLS-listener движка
```

Обе ветки идут с PROXY protocol, поэтому движок видит настоящий адрес
клиента, а не loopback. FakeTLS работает ровно как раньше: политика
неизвестного SNI и маскировка не меняются.

### Что нужно

- **Свой домен, отличный от домена маскировки.** Это обязательно: FakeTLS-клиент
  шлёт в SNI именно домен маскировки, и совпадение увело бы его в nginx вместо
  движка. По умолчанию берётся поддомен `web.<домен Selfmask>`.
...
- **nginx со stream и ssl_preread.** Обновить: `mtproxyl selfmask pq-install`.
- **Порт 443.** Telegram Desktop ходит в WEB только на него и порт в ссылку не пишет.
