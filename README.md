...
| `--cpus` | число, до числа ядер машины | без ограничений | Лимит CPU для контейнера. Больше, чем ядер, docker не примет — установка откажется заранее. |
| `--memory` | `512m`, `1g` | без ограничений | Лимит памяти для контейнера. |
