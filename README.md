...
7. Довозится `secrets.conf` целиком: лимиты, квоты, сроки и заметки аргументами
   не передать.
8. Панель переносится вместе с `config.toml` — логин, хеш пароля и `jwt_secret`
   остаются прежними, мастер настройки на новой машине не запускается. Бот
   ставится с прежним токеном и списком админов. Ответить заранее можно
   флагами `--panel`/`--no-panel` и `--tgbot`/`--no-tgbot`.
9. В конце предлагается удалить копию на старом сервере — по умолчанию нет:
...
> Если MTProxyL стоит из ветки, а не из `main`, панель на новой машине будет
> **собираться из исходников** в Docker — это несколько минут и заметная
> нагрузка. На маленькой машине сборка может съесть всю память. Переезд об
> этом предупреждает перед вопросом про панель.
