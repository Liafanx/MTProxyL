...
| Трафик, соединения, пользователи | из API цели `/v1/users`; метрики — из `metrics_listen`/`metrics_port` цели |
...
| Правка конфига | `edit-config` / меню «Цель / режим» — открывает конфиг цели в `$EDITOR`/nano, делает бэкап и предлагает рестарт только если файл реально изменился |
...
режим эксперта, бэкапы/миграция, upstream-маршруты и SNI-политика.
